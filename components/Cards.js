	...
	.then(result => {
        ...
        Object.values(data).forEach((articles) => {
            articles.forEach((article) => {
                // appending each card that is created to the main container to be displayed
                cardsContainer.append(createCards(article));
            })
        })
    })
    // when an error is found is will be console.log
	.catch(error => {
		console.log('An error has occurred', error)
	});

// selecting the cards container so everything can be appended to display on screen
...
function createCards(obj) {
    ...
    // event listener so when clicked it will console.log the article headline
